...
//! Copied and modified from Differential Dataflow's Prefix Sum, originally written by Frank McSherry

use differential_dataflow::{Collection, ExchangeData};
use differential_dataflow::lattice::Lattice;
use differential_dataflow::operators::{Iterate, Join, Threshold, Reduce};
use timely::dataflow::Scope;

/// Extension trait for the prefix_sum method.
pub trait PrefixSum2<G: Scope, K, D> {
    ...
    fn prefix_sum2<F>(&self, zero: D, combine: F) -> Self where F: Fn(&K,&D,&D)->D + 'static;

    /// Determine the prefix sum at each element of `location`.
    fn prefix_sum_at2<F>(&self, locations: Collection<G, (usize, K)>, zero: D, combine: F) -> Self where F: Fn(&K,&D,&D)->D + 'static;
}

impl<G, K, D> PrefixSum2<G, K, D> for Collection<G, ((usize, K), D)>
where
    G: Scope,
    G::Timestamp: Lattice,
...
{
    fn prefix_sum2<F>(&self, zero: D, combine: F) -> Self where F: Fn(&K,&D,&D)->D + 'static {
        self.prefix_sum_at2(self.map(|(x,_)| x), zero, combine)
    }
